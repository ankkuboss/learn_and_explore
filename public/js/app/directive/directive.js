...
app.directive("calcTime", function($compile) {
    ...
    return {
      ...
      link: function (scope, element, attrs) {
         ...
         var deadLine = attrs.calcTime; // setting purchase time Date Object
        var today = attrs.today; // setting purchase time Date Object
        //deadLine = new Date(deadLine.setDate(deadLine.getDate()+1)); // setting purchase time Date Object + 1 day
        timeDifference = deadLine - today; // deadline data minus current date
        ...
        if(diffDays > 0)
        {
        	 element.html(diffDays + " days");
        }
        else if(diffHours > 0 && diffMinutes > 0)
        {
        	 element.html(diffHours + " hours and "+ diffMinutes +" minutes") ;
        }
        else 
        {

        element.html( diffMinutes +" minutes") ;
        }
            }
    };  
});
...
app.directive('copyOnClick', ['CopyService','$rootScope', function (CopyService,$rootScope) {
	return {
		restrict: 'A',
		link: function (scope, element, attrs) {
			element.bind('click', function (e) {
				CopyService(attrs.copyOnClick);
				$rootScope.addAlert("success","Copy Success.");
			});
		}
	}
}]) 
